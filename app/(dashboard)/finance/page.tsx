'use client'

import { motion } from 'framer-motion'
import Link from 'next/link'
import { ArrowRight, AlertCircle, TrendingUp, Wallet, Receipt } from 'lucide-react'
import { RevenueSnapshot } from '@/components/overview/revenue-snapshot'
import { Collections } from '@/components/overview/collections'
import { AiInsights } from '@/components/shared/ai-insights'
import { Card, CardHeader, CardTitle, CardContent } from '@/components/ui/card'
import { Badge } from '@/components/ui/badge'
import { invoices } from '@/lib/mock-data'
import { USERS } from '@/lib/users'
import { getEmployeeProject, type AgentStatus } from '@/lib/employee-projects'
import { financeNextSteps } from '@/lib/ai-insights'
import { formatLakhs } from '@/lib/utils'

const EMPLOYEE_IDS = ['priya', 'rohan', 'divya', 'arjit', 'sneha', 'karan']

const agentBadge: Record<AgentStatus, { variant: 'green' | 'gold' | 'default' | 'red'; label: string }> = {
  done: { variant: 'green', label: 'Clear' },
  working: { variant: 'gold', label: 'Tracking' },
  queued: { variant: 'default', label: 'Queued' },
  'needs-input': { variant: 'red', label: 'Needs you' },
}

function StatCard({
  label,
  value,
  sub,
  icon: Icon,
  accent,
  index,
}: {
  label: string
  value: string
  sub?: string
  icon: typeof Wallet
  accent: string
  index: number
}) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 12 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.35, delay: index * 0.06 }}
    >
      <Card goldRule hover={false} className="p-5">
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs text-[var(--color-text-tertiary)] uppercase tracking-wider font-medium">
            {label}
          </p>
          <Icon size={15} style={{ color: accent }} />
        </div>
        <div
          className="text-2xl text-[var(--color-text-primary)]"
          style={{ fontFamily: 'var(--font-jetbrains-mono)', fontWeight: 400 }}
        >
          {value}
        </div>
        {sub && <p className="text-xs text-[var(--color-text-tertiary)] mt-1">{sub}</p>}
      </Card>
    </motion.div>
  )
}

export default function FinancePage() {
  const open = invoices.filter((i) => i.status !== 'paid')
  const totalOutstanding = open.reduce((s, i) => s + i.amount, 0)
  const overdue = invoices.filter((i) => i.status === 'overdue')
  const totalOverdue = overdue.reduce((s, i) => s + i.amount, 0)

  // The Finance Agent's state on every employee's campaign
  const financeStates = EMPLOYEE_IDS.map((id) => {
    const user = USERS.find((u) => u.id === id)!
    const project = getEmployeeProject(id)!
    const state = project.agentStates.find((a) => a.agentId === 'finance')!
    return { user, project, state }
  })
  const financeFlags = financeStates.filter((f) => f.state.status === 'needs-input').length

  return (
    <div className="p-4 sm:p-6 max-w-[1400px] mx-auto space-y-4 sm:space-y-6">
      {/* Header */}
      <motion.div
        initial={{ opacity: 0, y: -8 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.3 }}
        className="flex flex-wrap items-end justify-between gap-3"
      >
        <div>
          <h1
            className="text-2xl sm:text-3xl text-[var(--color-text-primary)] mb-1"
            style={{ fontFamily: 'var(--font-instrument-serif)' }}
          >
            Finance
          </h1>
          <p className="text-sm text-[var(--color-text-tertiary)]">
            Revenue, collections & billing · {open.length} invoices open
          </p>
        </div>
        {/* Run by the Finance Agent */}
        <div className="inline-flex items-center gap-2.5 rounded-[10px] border border-[var(--color-gold-border)] bg-[var(--color-gold-muted)] px-3 py-2">
          <div className="w-7 h-7 rounded-[8px] bg-[var(--color-gold)] flex items-center justify-center flex-shrink-0">
            <Receipt size={14} className="text-[var(--color-ink)]" />
          </div>
          <div className="leading-tight">
            <p className="text-[10px] uppercase tracking-wider text-[var(--color-text-tertiary)]">
              Run by the
            </p>
            <p className="text-xs text-[var(--color-text-primary)] font-medium">Finance Agent</p>
          </div>
        </div>
      </motion.div>

      {/* AI next steps */}
      <AiInsights steps={financeNextSteps} subtitle="Prioritised across collections & billing" />

      {/* Stat cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <StatCard index={0} label="Outstanding" value={formatLakhs(totalOutstanding)} sub="across all open invoices" icon={Wallet} accent="var(--color-gold)" />
        <StatCard index={1} label="Overdue" value={formatLakhs(totalOverdue)} sub={`${overdue.length} invoices past due`} icon={AlertCircle} accent="var(--color-status-red)" />
        <StatCard index={2} label="Q2 Revenue" value="₹92.4L" sub="↑ 8.4% vs last quarter" icon={TrendingUp} accent="var(--color-status-green)" />
        <StatCard index={3} label="Finance flags" value={`${financeFlags}`} sub="campaigns needing you" icon={Receipt} accent={financeFlags > 0 ? 'var(--color-status-red)' : 'var(--color-text-tertiary)'} />
      </div>

      {/* Revenue + Finance Agent across campaigns */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        <motion.div
          initial={{ opacity: 0, y: 16 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.4, delay: 0.1 }}
          className="lg:col-span-2"
        >
          <RevenueSnapshot />
        </motion.div>

        <motion.div
          initial={{ opacity: 0, y: 16 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.4, delay: 0.15 }}
        >
          <Card goldRule hover={false}>
            <CardHeader>
              <CardTitle>Finance Agent</CardTitle>
              <p className="text-xs text-[var(--color-text-tertiary)] mt-0.5">
                Billing status on every live campaign
              </p>
            </CardHeader>
            <CardContent className="pt-2 space-y-2">
              {financeStates.map(({ user, project, state }) => {
                const b = agentBadge[state.status]
                return (
                  <Link
                    key={user.id}
                    href={`/team/${user.id}`}
                    className="block rounded-[10px] border border-[var(--color-border-brand)] bg-[var(--color-surface-elevated)] p-3 hover:border-[var(--color-gold-border)] transition-colors duration-150"
                    style={state.status === 'needs-input' ? { background: 'var(--color-status-red-muted)' } : undefined}
                  >
                    <div className="flex items-center justify-between gap-2 mb-1">
                      <span className="text-sm text-[var(--color-text-primary)] font-medium">
                        {project.client}
                      </span>
                      <Badge variant={b.variant}>{b.label}</Badge>
                    </div>
                    <p className="text-xs text-[var(--color-text-tertiary)] leading-relaxed">
                      {state.output}
                    </p>
                  </Link>
                )
              })}
            </CardContent>
          </Card>
        </motion.div>
      </div>

      {/* Collections table */}
      <motion.div
        initial={{ opacity: 0, y: 16 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.4, delay: 0.2 }}
      >
        <Collections />
      </motion.div>
    </div>
  )
}
